...
class BuildOperationWorkerDaemonManagerTest extends Specification {
    def buildOperationWorkerRegistry = Mock(BuildOperationWorkerRegistry)
    def workerDaemonManager = Mock(WorkerDaemonFactory)
    def buildOperationWorkerDaemonManager = new BuildOperationWorkerDaemonManager(workerDaemonManager, buildOperationWorkerRegistry)
    ...
    def "build operation is started and finished when client is executed"() {
        ...
        when:
        buildOperationWorkerDaemonManager.getDaemon(Mock(WorkerDaemonProtocol).class, Mock(File), Mock(DaemonForkOptions), Mock(WorkerDaemonStarter)).execute(Mock(WorkerDaemonAction), Mock(WorkSpec))
